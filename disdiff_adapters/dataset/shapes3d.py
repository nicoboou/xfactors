...
class Shapes3DDataset(Dataset):
    ...
    def _apply_down_up_bilinear(self, image: torch.Tensor, level_norm: float) -> torch.Tensor:
        ...
        low = TF.resize(image, [low_h, low_w], interpolation=InterpolationMode.BILINEAR, antialias=True)
    ...
    def _apply_down_up_bicubic(self, image: torch.Tensor, level_norm: float) -> torch.Tensor:
        ...
        low_h = max(8, int(h * scale))
        low_w = max(8, int(w * scale))
        low = TF.resize(image, [low_h, low_w], interpolation=InterpolationMode.BICUBIC, antialias=True)
    ...
    def _apply_nearest_neighbor(self, image: torch.Tensor, level_norm: float) -> torch.Tensor:
        ...
        low_h = max(8, int(h * scale))
        low_w = max(8, int(w * scale))
        low = TF.resize(image, [low_h, low_w], interpolation=InterpolationMode.NEAREST, antialias=False)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        processed_img = self._apply_degradation(processed_img, level)
        ...
        return processed_img, target_with_degradation
