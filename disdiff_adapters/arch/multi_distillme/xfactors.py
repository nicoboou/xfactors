...
class Xfactors(LightningModule):
    ...
    def __init__(
        ...
        if map_idx_labels:
            assert isinstance(map_idx_labels, list), "If specified, map_idx_labels should be a list."
    ...
    def training_step(self, batch: tuple[torch.Tensor]):
        ...
        if torch.isnan(loss):
            ...
            try:
                loss = self.prev_loss.to(loss.device)
            except:
                ValueError("Nan loss")
        else:
    ...
    def generate_by_factors(
        ...
        # Fallback to full buffers when the requested value is absent from the current epoch sample.
        if factor_value != -1:
            ...
            if z_s.size(0) == 0:
                mask = torch.ones(buff_latents["t"].size(0), dtype=torch.bool)
                z_s = buff_latents["s"]
                z_t_left = buff_latents["t"][:, :start]
                z_t_right = buff_latents["t"][:, end:]
        else:
            mask = torch.ones(z_t.size(0), dtype=torch.bool)

        ref_pool_len = z_s.shape[0]
        if ref_pool_len == 0:
            raise ValueError("No latent samples available for factor-conditioned generation.")
        safe_pos = pos % ref_pool_len
        ...
        if img_ref is None:
            eps_s = torch.stack(nb_samples * [z_s[safe_pos]]).to(self.device)
            eps_t_left = torch.stack(nb_samples * [z_t_left[safe_pos]]).to(self.device)
            eps_t_right = torch.stack(nb_samples * [z_t_right[safe_pos]]).to(self.device)
        else:
            with torch.no_grad():
        ...
        if img_ref is not None:
            ref_img = img_ref.squeeze(0)
        else:
            ref_pool = buff_imgs[mask]
            if ref_pool.shape[0] == 0:
                ref_pool = buff_imgs
            ref_img = ref_pool[safe_pos % ref_pool.shape[0]]
    ...
    def generate_cond(
        ...
        # Logging can request a value absent from the current buffer; fallback to unfiltered buffers.
        if z_t.size(0) == 0 or z_s.size(0) == 0:
            mask = torch.ones(buff_latents["t"].size(0), dtype=bool)
            z_t = buff_latents["t"]
            z_s = buff_latents["s"]
        ...
        if cond == "t":
            ...
            if img_ref is None:
                safe_pos = pos % z_t.shape[0]
                eps_t = torch.stack(nb_samples * [z_t[safe_pos]]).to(self.device)
            else:
                with torch.no_grad():
        ...
        if img_ref is not None:
            ...
            if ref_pool.shape[0] == 0:
                ref_pool = buff_imgs
            safe_pos = pos % ref_pool.shape[0]
            ref_img = ref_pool[safe_pos]
    ...
    def show_reconstruct(self, images: torch.Tensor):
        ...
        for i in range(len(images)):
            ...
            images_gen_proc = (
                (255 * ((img_gen - img_gen.min()) / (img_gen.max() - img_gen.min() + 1e-8))).to("cpu", torch.uint8).permute(1, 2, 0).detach().numpy()
            )
    ...
    def _get_log_dir(self) -> str:
        ...
        candidates = [
            getattr(logger, "log_dir", None),
            getattr(logger, "save_dir", None),
            getattr(self.hparams, "log_dir", None),
        ...
        raise RuntimeError("Unable to resolve a valid logging directory.")

    def _log_val_artifacts_to_wandb(self, log_dir: str, epoch: int, cov_path: str):
        logger = getattr(self, "logger", None)
        experiment = getattr(logger, "experiment", None)
        if experiment is None or not hasattr(experiment, "log"):
            return

        val_dir = join(log_dir, f"epoch_{epoch}", "val")
        image_paths = {
            "val/reco": join(val_dir, f"reco_{epoch}.png"),
            "val/gen_all": join(val_dir, f"gen_all_{epoch}.png"),
            "val/cov": cov_path,
            "val/latent_space": join(val_dir, f"latent_space_{epoch}.png"),
        }
        ...
        if os.path.isdir(val_dir):
            ...
            for filename in sorted(os.listdir(val_dir)):
                if filename.startswith("merge_f=") and filename.endswith(".png"):
                    merge_path = join(val_dir, filename)
                    merge_images.append(wandb.Image(merge_path, caption=filename[:-4]))
            if merge_images:
                payload["val/merge"] = merge_images
    ...
    def on_train_epoch_end(self):
        ...
        if epoch % 1 == 0:
            os.makedirs(os.path.join(log_dir, f"epoch_{epoch}"), exist_ok=True)
    ...
    def on_validation_epoch_end(self):
        ...
        if epoch % 1 == 0:
            os.makedirs(os.path.join(log_dir, f"epoch_{epoch}"), exist_ok=True)
            os.makedirs(os.path.join(log_dir, f"epoch_{epoch}", "val"), exist_ok=True)
            ...
            path_heatmap = join(log_dir, f"epoch_{epoch}", "val", f"cov_{epoch}.png")
            ...
            self._log_val_artifacts_to_wandb(log_dir=log_dir, epoch=epoch, cov_path=path_heatmap)
    ...
    def log_reco(self, is_val: bool = False):
        ...
        save_reco_path = os.path.join(log_dir, f"epoch_{self.current_epoch}", f"reco_{self.current_epoch}.png")
        if is_val:
    ...
    def log_gen_images(self, is_val: bool = False):
        ...
        log_dir = self._get_log_dir()
        buff_latents = self.latent_val_buff if is_val else self.latent_train_buff
        latent_pool_size = buff_latents["s"].shape[0]
        if latent_pool_size == 0:
            raise ValueError("Latent pool is empty, cannot log generated images.")

        # save the generate images
        images_gen = self.generate(is_val=is_val)
        save_gen_path = join(log_dir, f"epoch_{epoch}", f"gen_{epoch}.png")
        if is_val:
            save_gen_path = join(log_dir, f"epoch_{epoch}", "val", f"gen_{epoch}.png")
        ...
        for i in range(4):
            ...
            save_gen_s_path = join(log_dir, f"epoch_{epoch}", f"gen_s_{epoch}_{i}.png")
            if is_val:
                save_gen_s_path = join(log_dir, f"epoch_{epoch}", "val", f"gen_s_{epoch}_{i}.png")
        ...
        for i in range(4):
            ...
            save_gen_t_path = join(log_dir, f"epoch_{epoch}", f"gen_t_{epoch}_{i}.png")
            if is_val:
                save_gen_t_path = join(log_dir, f"epoch_{epoch}", "val", f"gen_t_{epoch}_{i}.png")
        ...
        for cond in self.hparams.select_factors:
            for i in range(4):
                ...
                save_gen_f_path = join(log_dir, f"epoch_{epoch}", f"gen_f={cond}_{epoch}_{i}.png")
                if is_val:
        ...
        ### Merge in one image
        if is_val:
            path_epoch_s = [join(log_dir, f"epoch_{epoch}", "val", f"gen_s_{epoch}_{i}.png") for i in range(4)]
        else:
            path_epoch_s = [join(log_dir, f"epoch_{epoch}", f"gen_s_{epoch}_{i}.png") for i in range(4)]
        final_gen_s = merge_images_with_black_gap(path_epoch_s)

        if is_val:
            path_epoch_t = [join(log_dir, f"epoch_{epoch}", "val", f"gen_t_{epoch}_{i}.png") for i in range(4)]
        else:
            path_epoch_t = [join(log_dir, f"epoch_{epoch}", f"gen_t_{epoch}_{i}.png") for i in range(4)]
        ...
        for cond in self.hparams.select_factors:
            if is_val:
                ...
                path_epoch_f = [join(log_dir, f"epoch_{epoch}", f"gen_f={cond}_{epoch}_{i}.png") for i in range(4)]
            final_gen_f = merge_images_with_black_gap(path_epoch_f)
            final_gen_f.save(join(log_dir, f"final_gen_f={cond}.png"))
            for i in range(4):
                os.remove(path_epoch_f[i])
        final_gen_s.save(join(log_dir, "final_gen_s.png"))
        final_gen_t.save(join(log_dir, "final_gen_t.png"))

        map_ = self.hparams.map_idx_labels
        label = (
            [f"gen_{cond}" for cond in self.hparams.select_factors] if map_ is None else [f"gen_{map_[cond]}" for cond in self.hparams.select_factors]
        )
        final_image = merge_images(
            save_gen_path,
            join(log_dir, "final_gen_s.png"),
            join(log_dir, "final_gen_t.png"),
            *[join(log_dir, f"final_gen_f={cond}.png") for cond in self.hparams.select_factors],
            labels=["gen", "gen_s", "gen_t"] + label,
        )
        if is_val:
            save_gen_all_path = join(log_dir, f"epoch_{epoch}", "val", f"gen_all_{epoch}.png")
        else:
            save_gen_all_path = join(log_dir, f"epoch_{epoch}", f"gen_all_{epoch}.png")
        final_image.save(save_gen_all_path)

        os.remove(join(log_dir, "final_gen_s.png"))
        os.remove(join(log_dir, "final_gen_t.png"))
        for cond in self.hparams.select_factors:
            os.remove(join(log_dir, f"final_gen_f={cond}.png"))
        for i in range(4):
    ...
    def log_mosaic(self, is_val: bool = False, log_dir: str = ""):
        ...
        # This mosaic layout is CelebA-specific (hardcoded attribute indices).
        required_celeba_idx = (15, 20, 26, 31, 35, 36)
        if buff_labels.ndim != 2 or buff_labels.size(1) <= max(required_celeba_idx):
            return
        ...
        if masks["eyeglass"].sum() < 2 or masks["skin"].sum() < 3 or masks["male"].sum() < 4 or masks["smile"].sum() < 1:
            return
        ...
        for i in range(6):
            if i == 0:
                simg = buff_imgs[masks["eyeglass"]][0]
            elif i == 1:
                simg = buff_imgs[masks["skin"]][1]
            elif i == 2:
                simg = buff_imgs[masks["male"]][2]
            elif i == 4:
                simg = buff_imgs[masks["male"]][3]
            elif i == 3:
                simg = buff_imgs[masks["eyeglass"]][1]
            elif i == 5:
                simg = buff_imgs[masks["skin"]][2]
            else:
                pass
        ...
        for f in factors:
            for i, simg in enumerate(tiles):
                ...
                out = self.merge(
                    ...
                    tiles[-1].unsqueeze(0).to(self.device),
                    select_factor=f,
                )
                out = out.detach()
        ...
        grid = vutils.make_grid(grid_tensor, nrow=ncols, padding=padding, pad_value=pad_value)  # [C, Hgrid, Wgrid] in [0,1]
    ...
    def log_merge(self, is_val: bool = False, log_dir: str = ""):
        ...
        latent_pool_size = buff_latents["s"].shape[0]
        if latent_pool_size == 0:
            raise ValueError("Latent pool is empty, cannot log merge images.")
