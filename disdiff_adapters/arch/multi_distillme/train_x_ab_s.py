...
os.chdir("/projects/compures/nicolas/xfactors/disdiff_adapters")
sys.path.append("/projects/compures/nicolas/xfactors/disdiff_adapters/")
print(sys.path)
...
def main(flags: argparse.Namespace):
    ...
    match flags.dataset:
        case "dsprites":
            ...
            klw = 0.000001
            factor_value = -1
            select_factors = list(range(len(DSprites.Params.FACTORS_IN_ORDER) - 1))
        ...
        case "mpi3d":
            ...
            klw = 0.000001
            factor_value = -1
            select_factors = list(range(len(MPI3D.Params.FACTORS_IN_ORDER) - 1))
            n = len(select_factors)
            dims_by_factors = n * [2]
            l_nce_by_factors = n * [(1 / n) * 0.1]
        ...
        case "shapes":
            ...
            klw = 0.000001
            factor_value = -1
            select_factors = list(range(len(Shapes3D.Params.FACTORS_IN_ORDER) - 1))
            n = len(select_factors)
            dims_by_factors = n * [2]
            l_nce_by_factors = n * [(1 / n) * 0.1]
    ...
    version = f"{model_name}_epoch={flags.max_epochs}_beta=({flags.beta_t})_latent=({list_to_str(dims_by_factors)})_batch={flags.batch_size}_warm_up={warm_up}_lr={flags.lr}_arch={flags.arch}+l_cov={flags.l_cov}+l_nce={list_to_str(l_nce_by_factors)}+l_anti_nce={flags.l_anti_nce}_{flags.key}"
    print(f"\nVERSION : {version}\n")
