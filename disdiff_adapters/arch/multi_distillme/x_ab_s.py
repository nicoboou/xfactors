...
class _MultiDistillMe(torch.nn.Module):
    def __init__(self, in_channels: int, img_size: int, latent_dim_t: int, res_block: nn.Module = ResidualBlock):
...
class Xfactors(LightningModule):
    ...
    def __init__(
        ...
        self.model = _MultiDistillMe(
            in_channels=self.hparams.in_channels, img_size=self.hparams.img_size, latent_dim_t=sum(dims_by_factors), res_block=res_block
        )
    ...
    def training_step(self, batch: tuple[torch.Tensor]):
        ...
        loss = self.loss(mus_logvars_t, image_hat_logits, images, z_t, labels=labels, log_components=True)
    ...
    def generate_by_factors(self, cond: int, nb_samples: int = 16, pos: int = 0, z_t=None, is_val=False, img_ref=None, factor_value=-1):
