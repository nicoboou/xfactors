...
class MultiDistillMeModule(LightningModule):
    ...
    def training_step(self, batch: tuple[torch.Tensor]):
        ...
        loss = self.loss(
            mus_logvars_s, mus_logvars_t, image_hat_logits, images, z_s, z_t, labels=labels[:, self.hparams.select_factor], log_components=True
        )
    ...
    def show_reconstruct(self, images: torch.Tensor):
        ...
        for i in range(len(images)):
            ...
            images_gen_proc = (
                (255 * ((img_gen - img_gen.min()) / (img_gen.max() - img_gen.min() + 1e-8)))
                .to("cpu", torch.uint8)
                .permute(1, 2, 0)
                .detach()
                .cpu()
    ...
    def reload_latent(self, Class_=Shapes3D):
        ...
        for images in self.images_train_buff:
            if len(pill) < 4096:
                ...
                with torch.no_grad():
                    _, _, _, z_s, z_t, z = self.forward(images_batched.to(self.device), test=True)  # images shape : [32, 3, 64, 64]
                pill = []
                self.latent_train_buff["s"].append(z_s.detach().cpu())
                self.latent_train_buff["t"].append(z_t.detach().cpu())
    ...
    def log_factorvae(self, is_val: bool = False):
        ...
        if is_val:
            save_factorvae_path = os.path.join(self.logger.log_dir, f"epoch_{self.current_epoch}", "val", f"metric_{self.current_epoch}.json")
