...
class _VAE(torch.nn.Module):
    def __init__(self, in_channels: int, img_size: int, latent_dim: int, activation: nn.Module = nn.LeakyReLU, res_block: nn.Module = SimpleConv):

        super().__init__()

        self.encoder = Encoder(in_channels=in_channels, img_size=img_size, latent_dim=latent_dim, activation=activation, res_block=res_block)
