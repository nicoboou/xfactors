...
class SimpleConv(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(out_channels), nn.LeakyReLU()
        )
...
class Decoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not (self.img_size > 0 and (self.img_size & (self.img_size - 1))) == 0:
            x: torch.Tensor = F.interpolate(x, size=(self.img_size, self.img_size), mode="bilinear", align_corners=False)  # Padding
